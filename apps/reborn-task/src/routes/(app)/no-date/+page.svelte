<script lang="ts">
	...
	import { noDateTasks } from '$lib/stores/decrypted-tasks.store';
	...
	let allNoDate = $derived($noDateTasks ?? []);
	let activeTasks = $derived(allNoDate.filter((t: TaskListItem) => !t.is_completed));
		...
		{#if activeTasks.length === 0}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				...
				<p class="text-sm text-muted-foreground">
					{$t('taskList.filter.date.no_date')}
				</p>
			</div>
		{:else}
			<div class="space-y-2">

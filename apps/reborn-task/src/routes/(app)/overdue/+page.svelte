<script lang="ts">
	...
	import { overdueTasks } from '$lib/stores/decrypted-tasks.store';
	...
	let activeTasks = $derived(($overdueTasks ?? []).filter((t: TaskListItem) => !t.is_completed));
		...
		{#if activeTasks.length === 0}
			<div class="flex flex-col items-center justify-center py-12 text-center">
				...
				<p class="text-sm text-muted-foreground">
					{$t('taskList.filter.date.overdue')}
				</p>
			</div>
		{:else}
			<div class="space-y-2">

...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		if (username && loginLockout.isLocked(username)) {
		...
		if (result.success && result.data) {
			...
			if (result.data.user?.id) {
				...
				if (twoFactor) {
					// 2FA is enabled — return partial response, don't set cookies/tokens yet
					// Delete the refresh token that handleLogin already created
			...
			if (username) loginLockout.reset(username);
			...
			// Create UserSession for session history
			if (result.data.user?.id) {
				try {
			...
			if (username) loginLockout.recordFailure(username);

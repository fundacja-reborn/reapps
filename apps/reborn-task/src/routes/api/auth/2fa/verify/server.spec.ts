...
vi.mock('otpauth', () => {
	class MockTOTP {
		validate({ token }: { token: string }) {
			// Accept '123456' as valid TOTP for testing
...
const mockUser = {
	...
	twoFactorAuth: {
		is_enabled: true,
		secret_server: 'JBSWY3DPEHPK3PXP' // valid Base32
	}
};
...
describe('POST /api/auth/2fa/verify (reborn-task)', () => {
	...
	it('should accept valid recovery code with dash (ABCDE-FGHIJ)', async () => {
		...
		// Verify Prisma was queried with correctly normalized hash

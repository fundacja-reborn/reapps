<script lang="ts">
  ...
  let {
    ...
    open: boolean;
    /** When set, only shares with payload.source_id === sourceId are shown. */
    sourceId?: string | null;
    /** Custom dialog title. Defaults to a generic "Manage shares" copy. */
    title?: string;
    /** Optional: shows a "Create new share" CTA. Wired by caller. */
    onCreateNew?: () => void;
  } = $props();
  ...
  const visibleShares = $derived(
    storeState.shares.filter((s) => {
      // Server-side already filters per app; defense-in-depth on payload type.
      const isNote = s.snapshot_type === 'note'
        || (s.snapshot_type === 'unknown'
          && (decoded[s.id]?.payload.type === 'note' || decryptErrors.has(s.id)));
      if (!isNote) return false;
      if (sourceId === null) return true;
      // Per-source filter only matches shares we decrypted successfully (legacy
      // shares without a payload source_id never match).
...
<Dialog bind:open>
  <DialogContent class="flex max-h-[85vh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
    <DialogHeader class="flex-shrink-0 border-b px-6 py-4 pr-12">
      <DialogTitle>
        {title ?? (sourceId ? $t('share.manage.for_note_title') : $t('share.manage.dialog_title'))}
      </DialogTitle>
      {#if sourceId}
        <DialogDescription>
          {$t('share.manage.for_note_description')}
        </DialogDescription>
      {/if}
    </DialogHeader>
      ...
      {:else if visibleShares.length === 0}
        <Card>
          <CardContent class="px-4 py-8 text-center text-sm text-muted-foreground">
            {sourceId ? $t('share.manage.empty_for_note') : $t('share.list.empty')}
          </CardContent>
        </Card>
      {:else}

 ...
 *   const limiter = createRateLimiter({ maxRequests: 10, windowMs: 15 * 60_000 });
 *   // inside the handler:
 *   if (!limiter.check(request.headers.get('x-forwarded-for') ?? 'unknown')) {
...
export interface RateLimiter {
  ...
  /** Retry-After seconds remaining for a given IP (0 if not limited). */

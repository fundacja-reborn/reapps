...
| `SavedSearch.query_encrypted` / `SavedSearch.name_encrypted` | Ciphertext | Saved searches (smart folders) store the user's query string and display name encrypted. The server never sees which operators, tags, or phrases a saved view filters by; evaluation happens exclusively client-side through the same parser as live search. The server sees only **how many** saved searches exist and the optional `folder_id` they are pinned to (a FK already visible per the tree-structure row above). |
